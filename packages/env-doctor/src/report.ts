...
export function formatReport(exampleFile: string, envFile: string, diff: EnvDiffResult): string {
  ...
  if (diff.missing.length === 0 && diff.empty.length === 0 && diff.undocumented.length === 0) {
    lines.push("All good - .env matches .env.example with no missing, empty, or undocumented variables.");
  } else if (!diff.hasBlockingIssues) {
    lines.push("No blocking issues (only undocumented extras found).");
  } else {
    lines.push(`Blocking issues found: ${diff.missing.length} missing, ${diff.empty.length} empty.`);
  }
